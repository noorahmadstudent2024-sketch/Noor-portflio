...
(async () => {
  ...
  try {
    ...
    // Replace relative paths with file:// URLs for CSS and JS
    let modifiedHtml = htmlContent.replace(/href="styles\.css"/g, `href="file://${path.join(__dirname, 'styles.css')}"`);
    modifiedHtml = modifiedHtml.replace(/src="script\.js"/g, `src="file://${path.join(__dirname, 'script.js')}"`);
    ...
    if (formExists) {
      ...
      // Listen for dialog (alert) to confirm validation works
      ...
      // Submit again to ensure we catch the dialog
      await page.click('button[type="submit"]');
      ...
      if (!dialogAppeared) {
        console.log('⚠ Form validation might not be working properly');
      }
    } else {
